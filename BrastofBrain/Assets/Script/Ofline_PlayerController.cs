...
public class Ofline_PlayerController : MonoBehaviour,OfIDdamage

{
  ...
  private Animator m_animator; // アニメーター
    ...
    [SerializeField] private MagicDataBase magicDataBase; // 魔法のデータベース
    Magic magic; // 保存用
    ...
    public static float LMP = 0f; // 最大MP
    [SerializeField] private Slider mpslider; // MPスライダー
    ...
    private void Update()
    {
        ...
        //Debug.Log(gamepad);
            ...
            //攻撃した際の処理
            //マウスクリック
            if (Input.GetMouseButtonDown(1)&&magic.lostmp<=mp&&Offline_Shot.caun==false){ 
            ...
            //ジョイスティックのボタンを押す
            //この際の処理はマウスクリックと同じ
            if (Input.GetKeyDown ("joystick button 1")&&magic.lostmp<=mp&&Gamepad.current == null&&Offline_Shot.caun==false){ 
                mp=mp-magic.lostmp;
                m_animator.SetBool("Attack", true);
            }
            
            if (Input.GetKeyDown ("joystick button 2")&&magic.lostmp<=mp&&Gamepad.current == null&&Offline_Shot.caun==false){ 
                mp=mp-magic.lostmp;
                m_animator.SetBool("Attack", true);
            }
            ...
            if(Offline_Shot.caun==true&&Countertime>=0f){
                Countertime-=Time.deltaTime;//カウンターの発動時間を計測
            }
            if(Offline_Shot.caun==true&&0f>=Countertime){
               Offline_Shot.caun=false;//カウンターをoffに
            }

            //カウンターのゲージを増やす処理
            if(Offline_Shot.caun==false){
        ...
        if (Gamepad.current == null) //ゲームパットが認識されない場合
            return;            //処理終了
